...
- (void)setUpTitleViewsPosition {
    ...
    if (currentTitleView) {
        ...
        if (self.segmentStyle.selectedTitleFont) {
            currentTitleView.font = self.segmentStyle.selectedTitleFont;
        }
        if (self.segmentStyle.isShowImage) {
...
- (void)setupScrollLineAndCover {
    ...
    if (self.scrollLine) {
        ...
        // scrollLine 固定宽度
        CGFloat scrollLineWidth = self.segmentStyle.scrollLineWidth;
        if (scrollLineWidth > 0) {
            coverX += (coverW - scrollLineWidth) / 2.0;
            self.scrollLine.frame = CGRectMake(coverX , self.zj_height - self.segmentStyle.scrollLineHeight, scrollLineWidth , self.segmentStyle.scrollLineHeight);
        } else {
        ...
        // scrollLine 固定高度
        CGFloat scrollLineOffsetY = self.segmentStyle.scrollLineOffsetY;
        if (scrollLineOffsetY > 0) {
            self.scrollLine.zj_y = scrollLineOffsetY;
        }
    }
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            ...
            CGFloat scrollLineWidth = self.segmentStyle.scrollLineWidth;
            if (scrollLineWidth > 0) {
                weakSelf.scrollLine.zj_x = currentTitleView.zj_x + (currentTitleView.zj_width - weakSelf.segmentStyle.scrollLineWidth) / 2;
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.scrollLine) {
        ...
        CGFloat scrollLineWidth = self.segmentStyle.scrollLineWidth;
        if (scrollLineWidth > 0) {
            int oldWidth = oldTitleView.zj_width + wDistance * progress;
            
            self.scrollLine.zj_x = oldTitleView.zj_x + xDistance * progress + (oldWidth - self.segmentStyle.scrollLineWidth) / 2;
        } else {
            if (self.segmentStyle.isScrollTitle) {
...
- (void)showBadgeWithTitle:(NSString *)title badge:(NSInteger)badge {
    if (![title isKindOfClass:[NSString class]]) {
        return;
    }
    
    NSInteger index = [self.titles indexOfObject:title];
    [self showBadgeWithIndex:index badge:badge];
}

- (void)showBadgeWithIndex:(NSInteger)index badge:(NSInteger)badge {
    if (index >= self.titleViews.count) {
        return;
    }
    ZJTitleView *titleView = self.titleViews[index];
    if ([titleView isKindOfClass:[ZJTitleView class]]) {
        if (badge > 0) {
            //titleView.showBadge = YES;
            [titleView showBadge:YES number:badge style:1];
        } else {
            //titleView.showBadge = NO;
            [titleView showBadge:NO number:badge style:1];
        }
    }
}
...
- (UIView *)scrollLine {
    ...
    if (!_scrollLine) {
        ...
        UIImage *scrollImg = self.segmentStyle.scrollLineImage;
        if (scrollImg) {
            lineView.image = scrollImg;
        } else {

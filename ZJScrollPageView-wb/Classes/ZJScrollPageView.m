...
- (ZJContentView *)contentView {
    if (!_contentView) {
        NSInteger margin = self.segmentStyle.segmentAndContentMargin;
        ZJContentView *content = [[ZJContentView alloc] initWithFrame:CGRectMake(0.0, CGRectGetMaxY(self.segmentView.frame) + margin, self.bounds.size.width, self.bounds.size.height - CGRectGetMaxY(self.segmentView.frame) - margin) segmentView:self.segmentView parentViewController:self.parentViewController delegate:self.delegate];
...
- (ZJScrollSegmentView *)segmentView {
    if (!_segmentView) {
        ...
        ZJScrollSegmentView *segment = [[ZJScrollSegmentView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.segmentStyle.segmentHeight) segmentStyle:self.segmentStyle delegate:self.delegate titles:self.titlesArray titleDidClick:^(ZJTitleView *titleView, NSInteger index) {
            ...
            if (weakSelf.titleBtnOnClick) {
                weakSelf.titleBtnOnClick(titleView, index);
            }
